...
def main():
    ap=argparse.ArgumentParser(description="Export KITTI DEPTH (raw + depth_2 from .npy)")
    ap.add_argument("--root", required=True)
    ap.add_argument("--out", required=True)
    ap.add_argument("--anchor_camera", default="cam_zed_rgb")
    ap.add_argument("--depth_camera", default="cam_zed_depth")
    ap.add_argument("--depth_write_png16", action="store_true",
                    help="write 16-bit PNGs instead of .npy")
    ap.add_argument("--require_image", action="store_true")
    ap.add_argument("--require_lidar", action="store_true",
                    help="optional: include velodyne/ alongside depth")
